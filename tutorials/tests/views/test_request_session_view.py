...
from tutorials.models import User, Student, RequestedStudentSession, ProgrammingLanguage, TutorSession, Tutor
...
class RequestSessionViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.tutor_user = User.objects.get(username='@peterpickles')
    ...
    def test_get_request_session_redirects_when_admin(self):
    ...
    def test_successful_session_request_for_student(self):
        self.client.login(username=self.student_user.username, password='Password123')
    ...
    def test_unsuccessful_session_request_with_invalid_data_for_student(self):
        self.client.login(username=self.student_user.username, password='Password123')
        before_count = RequestedStudentSession.objects.count()
        response = self.client.post(self.url, {})
        after_count = RequestedStudentSession.objects.count()
    ...
    def test_successful_session_request_for_tutor(self):
        self.client.login(username=self.tutor_user.username, password='Password123')
        before_count = TutorSession.objects.count()
        response = self.client.post(self.url, {
            'programming_language': self.language.id,
            'level': 'beginner',
            'season': 'Fall',
        ...
        after_count = TutorSession.objects.count()
        self.assertEqual(after_count, before_count + 1)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'request_session.html')
        self.assertIn('message', response.context)
    ...
    def test_unsuccessful_session_request_with_invalid_data_for_tutor(self):
        self.client.login(username=self.tutor_user.username, password='Password123')
        before_count = TutorSession.objects.count()
        response = self.client.post(self.url, {})
        after_count = TutorSession.objects.count()
